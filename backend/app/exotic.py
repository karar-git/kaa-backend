...
1. **`inits.json`** — EXOTIC's initialisation file, filled in from our analysis:
   ...
   with `exotic -red -i inits.json -nea`.
2. **Pre-reduced light curve** — the four-column comma-separated file EXOTIC's
   ...
   its transit model to *our* photometry: `exotic -pre -i inits.json -nea`.
...
def inits(an: SessionAnalysis, obscode: str = "", fits_dir: str | None = None,
    ...
    return {
        "inits_guide": {
            ...
            "Comment1": "  exotic -red -i inits.json -nea      (reduce the raw frames with EXOTIC)",
            "Comment2": "  exotic -pre -i inits.json -nea      (fit EXOTIC's model to our light curve)",
...
Run EXOTIC on the raw frames
  ...
  3. exotic -red -i inits.json -nea

Fit EXOTIC's transit model to our light curve instead
  exotic -pre -i inits.json -nea
